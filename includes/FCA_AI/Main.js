...
express.set('DFP', (process.env.PORT || process.env.port || 82));
        ...
        switch (Boolean_Option.includes(key)) {
            ...
            case false: {
                switch (key) {
                    ...
                    case 'userAgent': {
                        globalOptions.userAgent = (options.userAgent || 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; SamsungBrowser/23.0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/138.0.7204.157 Safari/537.36');
                        break;
                    }
                    case 'proxy': {
...
function buildAPI(globalOptions, html, jar) {
    ...
    if (fb_dtsg) logger.Normal("Em đã bật auto login rồi nha");
    ...
    if (endpointMatch) {
        ...
        if (endpointMatch.input.includes("601051028565049")) {
            logger.Warning("Cảnh báo cảnh báo, để em");
            needWarningHandle = true;
        }
    } else {
        logger.Warning('Sử dụng MQTT mặc định');
    ...
    async function clearWarning(retries = 0) {
        try {
            ...
            if (data.data?.fb_scraping_warning_clear?.success) {
                logger.Normal("Em đã xử lí cảnh báo xong");
                logger.Normal("Em đang khởi động lại, chờ xíu");
            ...
            logger.Error("Em không xử lí cảnh báo được", e);
            return false;
        }
    }
    ...
    mainPromise = mainPromise
        ...
        .then(function(res) {
            ...
            if (!Regex_Via.test(res.body)) {
                globalOptions.userAgent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64; SamsungBrowser/23.0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/138.0.7204.157 Safari/537.36";
...
function setUserNameAndPassWord() {
    ...
    console.log(chalk.bold.hex('#F80000')("[</>]") + chalk.bold.hex('#FF7F00')(' => ') + "Operating System: " + chalk.bold.yellow(os.type()));
    console.log(chalk.bold.hex('#F80000')("[</>]") + chalk.bold.hex('#FF7F00')(' => ') + "Machine Version: " + chalk.bold.yellow(os.version()));
    console.log(chalk.bold.hex('#F80000')("[</>]") + chalk.bold.hex('#FF7F00')(' => ') + "Fca Version: " + chalk.bold.yellow(global.Fca.Version) + '\n');
    try {
...
function login(loginData, options, callback) {
    ...
    var globalOptions = {
        ...
        online: false,
        emitReady: false,
        userAgent: "Mozilla/5.0 (Windows NT 10.0; Win64; x64; SamsungBrowser/23.0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/138.0.7204.157 Safari/537.36"
		};
