...
try { ({ HttpsProxyAgent: HttpsProxyAgentCtor } = require('https-proxy-agent')); }
catch { HttpsProxyAgentCtor = require('https-proxy-agent'); }
...
function buildProxy() {
 ...
 if (!WebSocket_Global || WebSocket_Global.readyState !== WebSocket.OPEN) return next();
      let data;
  ...
  global.mqttClient.on('error', (err) => {
    ...
    if (ctx.globalOptions.autoReconnect) {
      ...
      globalCallback({
       ...
       if (ctx.globalOptions.allowExit)
    ...
    if (process.env.OnStatus === undefined) {
      ...
      if (Number(global.Fca.Require.FastConfig.AutoRestartMinutes) === 0) {
        ...
        setTimeout(() => {
          ...
          if (ctx.globalOptions.allowExit) process.exit(1);
...
catch (e) { return log.warn('listenMqtt: JSON parse error', e?.message); }
    if (topic === "/t_ms") {
      ...
      let payload;
try { payload = JSON.parse(jsonMessage.payload); }
catch (e) { return log.warn('listenMqtt: payload parse error', e?.message); } //'{"name":null,"step":[1,[1,[4,0,1,[5,"taskExists",[19,"415"]]],[23,[2,0],[1,[5,"replaceOptimsiticMessage","7192532113093667880","mid.$gABfX5li9LA6VdUymnWPRAdlkiawo"]]]],[1,[4,0,1,[5,"taskExists",[19,"415"]]],[23,[2,0],[1,[5,"mailboxTaskCompletionApiOnTaskCompletion",[19,"415"],true]]]],[1,[4,0,1,[5,"taskExists",[19,"415"]]],[23,[2,0],[1,[5,"removeTask",[19,"415"],[9]]]]]]}'
    ...
    if (ctx.callback_Task && ctx.callback_Task[request_ID] && ctx.callback_Task[request_ID].type) {
        ...
        const Data = getRespData(type, payload);
        if (!callback) {
          return;
        }
        else if (!Data) {
    ...
    const resolveAttachmentUrl = (i) => {
      if (!delta.attachments || i === delta.attachments.length || utils.getType(delta.attachments) !== 'Array') {
        ...
        if (fmtMsg) {
            ...
            if (global.Fca.Require.FastConfig.AntiGetInfo.AntiGetThreadInfo) {
             const current = Number(global.Fca.Data.MsgCount.get(fmtMsg.threadID) || 0);
global.Fca.Data.MsgCount.set(fmtMsg.threadID, current + 1);
            }    
...
function restartMqtt(api, callback) {
  try {
    if (ctx.mqttClient && ctx.mqttClient.end) {
      ctx.mqttClient.end(true); // force close
      ctx.mqttClient = null;
    }
    ctx.t_mqttCalled = false;
    ctx.syncToken = undefined;
    listenMqtt(defaultFuncs, api, ctx, (err, data) => {
      if (err) {
  ...
  topics.forEach(t => ctx.mqttClient.unsubscribe(t));
  try { ctx.mqttClient.publish("/browser_close", "{}"); } catch {}
  ctx.mqttClient.end(false, () => { ctx.mqttClient = undefined; });
}
...
var msgEmitter = new MessageEmitter();
    globalCallback = (callback || function(error, message) {
      if (error) {
        ...
        console.log("?? Error detected:", errorStr);

        // Uu tiên kiểm tra cảnh báo scraping trước
          ...
          defaultFuncs
            ...
            .then((response) => {
              try {
                ...
                if (res.errors) {
                  return log.error("FBScrapingWarning", "Không thể xáa cảnh báo Facebook.");
                }
                if (res?.data?.fb_scraping_warning_clear?.success) {
                  log.info("FBScrapingWarning", "đã xóa cảnh báo Facebook thành công. Khởi động lại MQTT sau 1 giây...");
                  setTimeout(() => { try { getSeqID(); } catch (e) {} }, 1000);
                }
              } catch (err) {
                log.error("FBScrapingWarning", "Lỗi phân tích JSON: " + err.message);
              }
            })
            .catch((err) => {
              log.error("FBScrapingWarning", "Lỗi gọi HTTP POST: " + err.message);
            });
        } else if (errorStr.includes("Not logged in") || !ctx.loggedIn) {
          log.error("LoginError", "Phiên đăng nhâp không hợp lệ hoặc đã hết hạn.");
          return msgEmitter.emit("error", { error: "Not logged in" });
        } else {
